...
if((stripos($mtext,"Zo‘r") !== false) or (stripos($mtext,"yaxshi")!==false) or (stripos($mtext,"Zor")!==false) or (stripos($mtext,"Zo'r")!==false)){
  ...
  $input = array("Qayerdansiz asalim?","Juda  yaxshi asalim😁","👍","Ok asalim.","Qaysi viloyatdansiz asalim?", "Nima uchun asalim","Har doim shunday bo'lsin asalim.","Qayerliksiz asalim?");
  $rand=rand(0,7);
  $soz="$input[$rand]";
  $a=json_encode(bot('sendmessage',[
   'chat_id'=>$chat_id,
...
$input = array("Ha","Ha shunaqa","Hm shunday","Haye.","Ha rost");
 ...
 if($fadmin==$admin){
    bot('sendmessage',[
      'chat_id'=>$chat_id,
      'text'=>"Vay mana meni yaratgan inson keldilar 😘 @Zed_blogger cham😉.",
      ]);
  }else{
  $name = $message->from->first_name;
...
if(stripos($mtext,"kimsan")!== false){
  $name = $message->from->first_name;
  $input = array("Ammangni qiziman😂","Poxxuy emasmi 😂.","Hazilkash robotcha!","kim deb o‘ylaysiz?","@Tg_Qizlar kanalining adminkasiz Bekorchi_Ro_bot man😜","Kapitan Telegram","Qitmirxonman, yana Masha bilan adashtirib yubormeng😂","Sirliman🎩","O‘ziz kimsiz?","Har doim shu savolni  beruvirib charchamadizmi?","Tinchlikmi kimligim bilan qiziqib qolibsiz!?","Menmi?","Nima edi😡...Vux qo‘rqib ketdiza?","Selena Gomes,","Aytsam sizni oldirishga togri keladi");
  $rand=rand(0,13);
  $soz="$input[$rand]";
  $a=json_encode(bot('sendmessage',[
   'reply_to_message_id'=>$mesid,
...
if((mb_stripos($mtext,"#zed") !== false) or (mb_stripos($mtext,"zed")!==false) or (stripos($mtext,"blogger")!==false) or (mb_stripos($mtext,"admin")!==false) or (mb_stripos($mtext,"зед")!==false)){ 
bot('SendMessage',[
'chat_id'=>$admin,
'parse_mode'=>'html',
